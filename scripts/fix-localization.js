...
const variations = {
  de: {
    intro: [
      (city, category) =>
        `Professionelle ${category.toLowerCase()}-Lösungen jetzt in ${city} verfügbar.`,
      (city, category) =>
        `${city} - Ihr Partner für erstklassige ${category.toLowerCase()}-Services.`,
      (city, category) =>
        `Maßgeschneiderte ${category.toLowerCase()}-Dienstleistungen speziell für ${city}.`,
      (city, category) =>
        `${city}: Wo Innovation auf bewährte ${category.toLowerCase()}-Expertise trifft.`,
      (city, category) =>
        `Entdecken Sie führende ${category.toLowerCase()}-Services in ${city}.`,
    ],
    whyChoose: [
  ...
  fr: {
    intro: [
      (city, category) =>
        `Solutions professionnelles de ${category.toLowerCase()} maintenant disponibles à ${city}.`,
      (city, category) =>
        `${city} - Votre partenaire pour des services ${category.toLowerCase()} de premier plan.`,
      (city, category) =>
        `Services ${category.toLowerCase()} sur mesure spécialement pour ${city}.`,
      (city, category) =>
        `${city} : Où l'innovation rencontre l'expertise ${category.toLowerCase()} éprouvée.`,
      (city, category) =>
        `Découvrez les services ${category.toLowerCase()} de pointe à ${city}.`,
    ],
    whyChoose: [
  ...
  ar: {
    intro: [
      ...
      (city, category) =>
        `${city}: حيث تلتقي الابتكار بخبرة ${category} المؤكدة.`,
...
function generateLocalizedContent(locale, country, state, city, category) {
  ...
  const whyChoose = getVariationText(
    locale,
    "whyChoose",
    seed + 1,
    city,
  ...
  const benefits = getVariationText(
    locale,
    "benefits",
    seed + 2,
    city,
  ...
  const timeline = getVariationText(
    locale,
    "timeline",
    seed + 3,
    city,
  ...
  const expertise = getVariationText(
    locale,
    "expertise",
    seed + 4,
    city,
  ...
  if (locale === "ar") {
    ...
    localizedPrice = `${sarMin.toLocaleString("ar-SA")}–${sarMax.toLocaleString(
      "ar-SA"
    )} ر.س`;
  } else {
    localizedPrice = `${finalMin.toLocaleString(
      locale
    )}–${finalMax.toLocaleString(locale)}${localeData.currency}`;
  }
...
function createMDXContent(locale, country, state, city, category, content) {
  ...
  const canonicalUrl = `https://moydus.com/${country}/${state}/${city}/${category}`;
  const title =
    locale === "ar"
...
alternates:
  canonical: "${canonicalUrl}"
  languages:
    en: "https://moydus.com/${country}/${state}/${city}/${category}"
openGraph:
  title: "${title}"
  description: "${description}"
  url: "${canonicalUrl}"
...
- ${
    locale === "ar"
      ...
      : "Fortschrittliches responsives Design"
  }
- ${
    locale === "ar"
...
${
  locale === "ar"
    ? "اتصل بنا للحصول على استشارة مجانية وعرض أسعار مخصص لمشروعك."
    : locale === "fr"
    ? "Contactez-nous pour une consultation gratuite et un devis personnalisé pour votre projet."
    : "Kontaktieren Sie uns für eine kostenlose Beratung und ein individuelles Angebot für Ihr Projekt."
}
      ...
      if (stat.isDirectory()) {
        ...
        if (pathParts.length >= 4) {
          ...
          const category = pathParts[4]
            ? pathParts[4].replace(".mdx", "")
            : item.replace(".mdx", "");

          // Generate localized content
          const content = generateLocalizedContent(
            locale,
            country,
            state,
            city,
