interface SnippetCalloutProps {
  title: string;
  children: React.ReactNode;
  variant?: "tip" | "warning" | "note" | "success" | "important";
...
export function SnippetCallout({
  ...
  const variants = {
    ...
    warning: {
      ...
      icon: "text-yellow-600 dark:text-yellow-400/80",
    },
    note: {
      ...
      icon: "text-gray-600 dark:text-gray-400/80",
    },
    success: {
  ...
  return (
    <div
      className={`my-4 px-5 py-4 overflow-hidden rounded-2xl flex gap-3 border ${config.border} ${config.bg}`}
    ...
    >
      ...
      <div
        className={`text-sm prose min-w-0 w-full ${config.text}`}
        data-component-part="callout-content"
      >
        <h3
          className={`text-base font-semibold mb-1 ${config.text}`}
          {...(snippetOptimized && {
            itemProp: "name",
          })}
        >
...
// Quick access variants for common use cases
export function QuickCallout({ children }: { children: React.ReactNode }) {
  return (
    <SnippetCallout title="Quick Tip" variant="tip">
      {children}
    </SnippetCallout>
  );
}
...
export function WarningCallout({ children }: { children: React.ReactNode }) {
  return (
    <SnippetCallout title="Important Warning" variant="warning">
      {children}
    </SnippetCallout>
  );
}

export function ProCallout({ children }: { children: React.ReactNode }) {
  return (
    <SnippetCallout title="Pro Tip" variant="success">
      {children}
    </SnippetCallout>
  );
}
